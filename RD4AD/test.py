...
def evaluation(encoder, bn, decoder, dataloader,device,_class_=None):
    ...
    with torch.no_grad():
        for img, _, label, _, gt in tqdm(dataloader, desc=f'{_class_} Testing'):
            img = img.to(device)
            inputs = encoder(img)
            ...
            y_pred.append(np.max(anomaly_map))
            for im in img:
                y_image.append(wandb.Image(im.permute(1, 2, 0).cpu().numpy()))
        ...
        preds = np.concatenate((y_pred.reshape((-1, 1)), y_true.reshape((-1, 1)), y_image.reshape((-1, 1))), axis=1)
        table = wandb.Table(data=preds, columns=['total_pred', 'total_label', 'image'])

        labels = [f'{_class_}_normal', f'{_class_}_defect']
        binary_pred = np.concatenate(((1 - y_pred).reshape((-1, 1)), y_pred.reshape((-1, 1))), axis=1)
        roc = wandb.plot.roc_curve(y_true, binary_pred, labels=labels)
        pr = wandb.plot.pr_curve(y_true, binary_pred, labels=labels)
        ...
        try:
            ...
            auroc_sp = 0.0
    return auroc_px, auroc_sp, round(np.mean(aupro_list),3), table, roc, pr, y_true, y_pred, y_image
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        me = mean(pros) if len(pros) != 0 else 0.0
        metrics.append(pd.DataFrame({"pro": me, "fpr": fpr, "threshold": th}, index=[0]))
