...
def test(
    ...
    load_test_checkpoint(model, model_type, model_path)

    tokenizer = open_clip.get_tokenizer('ViT-B-16-plus-240')

    dataset, _ = construct_test_loader(dataset_name, dataset_path, transform, category=None)

    few_shot_set = {}
    shot = 0
    for c in dataset.categories:
        few_shot_path = os.path.join(fshot_path, c + ".pt")
    ...
    loaders = construct_test_loaders(dataset_name, dataset_path, transform)
    ...
    avg_roc_auc = statistics.mean(roc_aucs)
    avg_auc_pr = statistics.mean(auc_prs)
    wandb.log({'avg_roc_auc': avg_roc_auc, 'avg_auc_pr': avg_auc_pr})
    ...
    wandb.log({'PR': wandb.plot.pr_curve(np.hstack(all_labels), all_preds, labels=['normal', 'defectious'])})

    wandb.finish()

    return avg_roc_auc, avg_auc_pr 
...
if __name__ == '__main__':
    ...
    test(dataset_name=args.dataset_name, dataset_path=args.dataset_path,
         model_type=args.model_type, model_path=args.model_path, fshot_path=args.few_shot_path)
